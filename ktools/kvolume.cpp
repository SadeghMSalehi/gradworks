...
vtkDataSet* createGrid(vtkPolyData* osurf, vtkPolyData* isurf, const int dims, size_t& insideCountOut) {
	...
	struct GridCreate {
		...
		void createImage(vtkImageData* im) {
			...
			im->SetExtent(extent);
			im->SetOrigin(origin);
			im->SetScalarTypeToInt();
			im->AllocateScalars();
			im->GetPointData()->GetScalars()->FillComponent(0, 255);
		}
		
		vtkImageData* createStencil(vtkImageData* im, vtkPolyData* surf) {
			createImage(im);
			...
			isten->SetInput(im);
			isten->SetStencil(psten->GetOutput());
			isten->ReverseStencilOff();
			isten->SetBackgroundValue(0);
			isten->Update();
			
			vtkImageData* output = isten->GetOutput();
	...
	vtkImageData* goim = gc.createStencil(gim.GetPointer(), osurf);
	vtkImageData* woim = gc.createStencil(wim.GetPointer(), isurf);
	
	struct BoundaryCheck {
		size_t subtract(vtkImageData* aim, vtkImageData* bim) {
...
void runFillGrid(Options& opts, StringVector& args) {
	if (opts.GetBool("-humanBrain")) {
		...
		vtkImageData* imgGrid = vtkImageData::SafeDownCast(grid);
		imgGrid->GetPointData()->GetScalars()->SetName("SampledValue");
		vtkNew<vtkImageToStructuredGrid> imgToGrid;
		imgToGrid->SetInput(imgGrid);
		imgToGrid->Update();
		
		vio.writeFile(outputFile, imgToGrid->GetOutput());
	...
	public:
		...
		void computeStep() {
			...
			for (size_t j = 0; j < nPts; j++) {
				...
				Geometry::Neighbors::const_iterator iter = edgeMap.cbegin();
...
void runEnclosingSphere(Options& opts, StringVector& args) {
	...
	sphereSource->SetRadius(radius-3);
