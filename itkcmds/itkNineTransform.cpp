...
public:
    ...
    Metric::FixedImageIndexContainer labelIndexes;
    ...
    void LoadFiles(int argc, char* argv[]) {
        ...
        for (labelIter.GoToBegin(); !labelIter.IsAtEnd(); ++labelIter) {
            ...
            if (label > 0) {
                labelIndexes.push_back(labelIter.GetIndex());
            }
        }
    ...
    void RunRigid() {
        ...
        metric->SetFixedImageIndexes(labelIndexes);
...
//        metric->SetUseAllPixels(true);
        ...
        opti->SetMaximumIteration(300);
        opti->SetUseUnitLengthGradient(false);
        opti->SetScales(scales);
