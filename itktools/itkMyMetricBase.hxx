 ...
 *
 *=========================================================================*/
#ifndef __itkMyMetricBase_hxx
#define __itkMyMetricBase_hxx
...
MyMetricBase<TFixedImage, TMovingImage>::MyMetricBase() :
		m_UseFixedImageIndexes(false), m_FixedImageIndexes(0),

		m_UseFixedImageSamplesIntensityThreshold(false), m_FixedImageSamplesIntensityThreshold(
				0),

		m_FixedImageSamples(0), m_NumberOfParameters(0),
		...
		m_FixedImageMask(0), m_MovingImageMask(0),

		m_NumberOfThreads(1),

		m_UseAllPixels(false), m_UseSequentialSampling(false), m_ReseedIterator(
				false), m_RandomSeed(-1),

		m_TransformIsBSpline(false), m_NumBSplineWeights(0),

		m_BSplineTransform(NULL), m_BSplineTransformWeightsArray(), m_BSplineTransformIndicesArray(), m_BSplinePreTransformPointsArray(
				0), m_WithinBSplineSupportRegionArray(0), m_BSplineParametersOffset(),

		m_UseCachingOfBSplineWeights(true), m_BSplineTransformWeights(), m_BSplineTransformIndices(), m_ThreaderBSplineTransformWeights(
				NULL), m_ThreaderBSplineTransformIndices(NULL),

		m_InterpolatorIsBSpline(false), m_BSplineInterpolator(NULL), m_DerivativeCalculator(
				NULL),

		m_Threader(MultiThreaderType::New()), m_ThreaderNumberOfMovingImageSamples(
				NULL), m_WithinThreadPreProcess(false), m_WithinThreadPostProcess(
				false) {
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SetNumberOfThreads(
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SetTransformParameters(
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SetNumberOfFixedImageSamples(
	...
	if (numSamples != m_NumberOfFixedImageSamples) {
		...
		if (m_NumberOfFixedImageSamples
				!= this->m_FixedImageRegion.GetNumberOfPixels()) {
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SetFixedImageIndexes(
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SetUseFixedImageIndexes(
		bool useIndexes) {
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SetFixedImageSamplesIntensityThreshold(
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SetUseFixedImageSamplesIntensityThreshold(
		bool useThresh) {
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SetUseAllPixels(
		bool useAllPixels) {
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SetUseSequentialSampling(
		bool useSequential) {
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::Initialize(void)
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::MultiThreadingInitialize(void)
		throw (ExceptionObject) {
	...
	m_ThreaderNumberOfMovingImageSamples = new unsigned int[m_NumberOfThreads
			- 1];
	...
	if (this->m_TransformIsBSpline) {
		...
		if (this->m_UseCachingOfBSplineWeights) {
			...
			m_BSplineTransformIndicesArray.SetSize(m_NumberOfFixedImageSamples,
					m_NumBSplineWeights);
			m_BSplinePreTransformPointsArray.resize(
					m_NumberOfFixedImageSamples);
			m_WithinBSplineSupportRegionArray.resize(
					m_NumberOfFixedImageSamples);
			...
			this->m_ThreaderBSplineTransformWeights =
					new BSplineTransformWeightsType[m_NumberOfThreads - 1];
			this->m_ThreaderBSplineTransformIndices =
					new BSplineTransformIndexArrayType[m_NumberOfThreads - 1];

			for (ThreadIdType ithread = 0; ithread < m_NumberOfThreads - 1;
					++ithread) {
				this->m_ThreaderBSplineTransformWeights[ithread].SetSize(
						this->m_NumBSplineWeights);
				this->m_ThreaderBSplineTransformIndices[ithread].SetSize(
						this->m_NumBSplineWeights);
			}
		}

		for (unsigned int j = 0; j < FixedImageDimension; j++) {
			this->m_BSplineParametersOffset[j] =
					j
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SampleFixedImageIndexes(
		FixedImageSampleContainer & samples) const {
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SampleFixedImageRegion(
		FixedImageSampleContainer & samples) const {
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SampleFullFixedImageRegion(
		FixedImageSampleContainer & samples) const {
	if (samples.size() != m_NumberOfFixedImageSamples) {
		throw ExceptionObject(__FILE__, __LINE__,
				"Sample size does not match desired number of samples");
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::ComputeGradient() {
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::ReinitializeSeed() {
	Statistics::MersenneTwisterRandomVariateGenerator::GetInstance()->SetSeed();
}

// Method to reinitialize the seed of the random number generator
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::ReinitializeSeed(int seed) {
	Statistics::MersenneTwisterRandomVariateGenerator::GetInstance()->SetSeed(
			seed);
}
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::PreComputeTransformValues() {
	...
	for (fiter = m_FixedImageSamples.begin(); fiter != fend;
			++fiter, counter++) {
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::TransformPoint(
		unsigned int sampleNumber, MovingImagePointType & mappedPoint,
		bool & sampleOk, double & movingImageValue,
	...
	if (!m_TransformIsBSpline) {
		...
		typedef itk::MatrixOffsetTransformBase<double,
				MovingImageType::ImageDimension, MovingImageType::ImageDimension> MatrixTransformType;
		MatrixTransformType* matrixTransform =
				dynamic_cast<MatrixTransformType*>(transform);
		if (matrixTransform) {
			// cout << "Transform Matrix: " << endl << dynamic_cast<MatrixTransformType*>(transform)->GetMatrix() << endl;
		}
	} else {
		if (this->m_UseCachingOfBSplineWeights) {
			...
			if (threadID > 0) {
				weightsHelper =
						&(this->m_ThreaderBSplineTransformWeights[threadID - 1]);
				indicesHelper =
						&(this->m_ThreaderBSplineTransformIndices[threadID - 1]);
			} else {
			...
			this->m_BSplineTransform->TransformPoint(
					this->m_FixedImageSamples[sampleNumber].point, mappedPoint,
					*weightsHelper, *indicesHelper, sampleOk);
		}
	}
	...
	if (sampleOk) {
		...
		if (m_InterpolatorIsBSpline) {
			...
			sampleOk = sampleOk
					&& m_BSplineInterpolator->IsInsideBuffer(mappedPoint);
			if (sampleOk) {
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::TransformPointWithDerivatives(
		unsigned int sampleNumber, MovingImagePointType & mappedPoint,
		bool & sampleOk, double & movingImageValue,
	...
	if (!m_TransformIsBSpline) {
		...
		if (this->m_UseCachingOfBSplineWeights) {
			...
			if (sampleOk) {
				...
				const ParametersType &Local_Parameters =
		...
		if (m_InterpolatorIsBSpline) {
			...
			if (sampleOk) {
				this->m_BSplineInterpolator->EvaluateValueAndDerivative(
						mappedPoint, movingImageValue, movingImageGradient,
						threadID);
			}
		} else {
			// Check if mapped point inside image buffer
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::ComputeImageDerivatives(
		const MovingImagePointType & mappedPoint,
		ImageDerivativesType & gradient, ThreadIdType threadID) const {
	if (m_InterpolatorIsBSpline) {
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::GetValueMultiThreadedPreProcessInitiate(
		void) const {
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::GetValueMultiThreadedInitiate(
		void) const {
	this->SynchronizeTransforms();
	...
	for (ThreadIdType threadID = 0; threadID < m_NumberOfThreads - 1;
			threadID++) {
		this->m_NumberOfPixelsCounted +=
				m_ThreaderNumberOfMovingImageSamples[threadID];
	}
}

template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::GetValueMultiThreadedPostProcessInitiate(
		void) const {
 ...
 */
template<class TFixedImage, class TMovingImage>
ITK_THREAD_RETURN_TYPE MyMetricBase< TFixedImage, TMovingImage >
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::GetValueThread(
		ThreadIdType threadID) const {
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::GetValueAndDerivativeMultiThreadedPreProcessInitiate(
		void) const {
	this->SynchronizeTransforms();
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::GetValueAndDerivativeMultiThreadedInitiate(
		void) const {
	this->SynchronizeTransforms();
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::GetValueAndDerivativeMultiThreadedPostProcessInitiate(
		void) const {
...
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::GetValueAndDerivativeThread(
		ThreadIdType threadID) const {
	// Figure out how many samples to process
	int chunkSize = m_NumberOfFixedImageSamples / m_NumberOfThreads;
	...
	for (int count = 0; count < chunkSize; ++count, ++fixedImageSample) {
		...
		TransformPointWithDerivatives(fixedImageSample, mappedPoint, sampleOk,
				movingImageValue, movingImageGradientValue, threadID);

		if (sampleOk) {
			// CALL USER FUNCTION
			if (this->GetValueAndDerivativeThreadProcessSample(threadID,
					fixedImageSample, mappedPoint, movingImageValue,
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::PrintSelf(std::ostream & os,
		Indent indent) const {
	...
	if (m_ThreaderNumberOfMovingImageSamples) {
		for (ThreadIdType i = 0; i < m_NumberOfThreads - 1; i++) {
			os << "  Thread[" << i << "]= "
					<< (unsigned int) m_ThreaderNumberOfMovingImageSamples[i]
					<< std::endl;
		}
	}
	...
	os << indent << "Fixed Image Mask: " << m_FixedImageMask.GetPointer()
			<< std::endl;
	os << indent << "Number of Moving Image Samples: "
			<< m_NumberOfPixelsCounted << std::endl;
 ...
 */
template<class TFixedImage, class TMovingImage>
void MyMetricBase<TFixedImage, TMovingImage>::SynchronizeTransforms() const {
	for (ThreadIdType threadID = 0; threadID < m_NumberOfThreads - 1;
			threadID++) {
		/** Set the fixed parameters first. Some transforms have parameters which depend on
		 the values of the fixed parameters. For instance, the BSplineTransform
		 checks the grid size (part of the fixed parameters) before setting the parameters. */
		this->m_ThreaderTransform[threadID]->SetFixedParameters(
				this->m_Transform->GetFixedParameters());
		this->m_ThreaderTransform[threadID]->SetParameters(
				this->m_Transform->GetParameters());
	}
}
} // end namespace itk
