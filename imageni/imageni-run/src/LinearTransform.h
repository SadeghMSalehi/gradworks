...
union TransformParamType {
	...
	void linearCombination(float a, TransformParamType& b,
			TransformParamType& c) {
		for (int i = 0; i < 8; i++) {
...
public:
	...
	static void createEulerAngleTransform(float x, float y, float z,
			Mat4& out) {
		float rx = x * PI / 180.0f;
		float ry = y * PI / 180.0f;
		float rz = z * PI / 180.0f;
	...
	/**
		 ...
		 */
		static void createRigidTransformForImage(TransformParamType& param,
				int w, int h, Mat3& out) {
			float scale = 1;
			float theta = param._x[2] * PI / 180.f;
			float tx = param._x[0];
			float ty = param._x[1];
	 ...
	 * transform ij-coordinate to xy-coordinate
	 */
	static void createSimilarityTransformForImage(TransformParamType& param,
			int w, int h, Mat3& out) {
		float scale = param._x[3] / 100.f;
		float theta = param._x[2] * PI / 180.f;
		float tx = param._x[0];
...
public:
	...
	bool resampleIJ2XY(const IntImage& src, const IntImage& obj, Mat3 fTx,
			IntImage& dst) {
	...
	bool resampleIJ2XYinReal(const CImage<SrcType>& src, const CImage<SrcType>& obj, Mat3 fTx,
				CImage<DstType>& dst) {
			fTx.suppressEpsilion();
			if (!dst.reuseOrCreateEmptyOf(obj.getWidth(), obj.getHeight())) {
			...
			for (ij[1] = 0; ij[1] < obj.getHeight(); ij[1]++) {
				for (ij[0] = 0; ij[0] < obj.getWidth(); ij[0]++) {
					mult(fTx, ij, xy);
					if (src.isInside(xy[0], xy[1])) {
						dst(ij[0], ij[1]) = static_cast<DstType>(src.bilinearFloat(xy[0], xy[1]));
					} else {
						dst(ij[0], ij[1]) = 0;
					}
				}
	...
	void extract(const IntImage& src, const IntImage& obj, Mat3 fTx,
			IntImage& dst) {
		fTx.suppressEpsilion();
	...
	void resampleH(IntImage& src, Mat4& fTx, IntImage& dst) {
		...
		Mat4 view, txf;
		ImageTransform::createImageToNormalizedCoordinate(dst.getWidth(),
				dst.getHeight(), view);
		mult(bTx, view, txf);
	...
	void projectH(IntImage& src, Mat4& fTx, IntImage& dst) {
		Mat4 view, txf;
		ImageTransform::createNormalizedToImageCoordinate(dst.getWidth(),
				dst.getHeight(), view);
		mult(view, fTx, txf);
