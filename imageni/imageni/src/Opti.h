...
public:
    ...
    int optimize(InputType& x0, InputType& out) {
        ...
        for (int i = 0; i < _maxIter; i++) {
            ...
            float f1;
            _fun->value(f1);
            //_DBG_(cout << "iteration: i = " << i << "; x = " << x << " => " << x1 << "; dx = " << g << "; step = " << _step << endl);
            change = f1 - f;
            if (abs(change) < _eps || change > 0) {
                xs = x;
                //_DBG_(cout << "solution: xs = " << xs << "; f = " << f << "; f1 = " << f1 << "; df = " << change << endl);
                break;
            } else {
                f = f1;
                x = x1;
