...
#ifndef TRACKEROLD_H_
#define TRACKEROLD_H_
...
private:
	...
	int __track(TransformParamType x0, Mat3& txfOut,
			TransformParamType& xsOut) {
		CSSD<Pixel> func(_frame, _object);
		CGradientDescent<CSSD<Pixel> > opti(&func);
		int elapsedTime = opti.optimize(x0, xsOut);
		func.createTransform(xsOut, txfOut);
		return elapsedTime;
	}

public:
	CTracker(ImageType* object) {
		_object = object;
		_w = _object->getWidth();
		_h = _object->getHeight();
		_report = NULL;
		_frame = NULL;
	}
