...
//  m_OutputSpacing.Fill( 1 ) ;
  ...
  this->m_Jacobian.SetSize( 3 , 3 ) ;
	m_UseDisplacementAsTranslation = 1;
}
...
::TransformPoint( const InputPointType & inputPoint ) const
{
   ...
   displacement = m_DeformationField->GetPixel( index ) ;
	 ...
	 for (int d = 0; d < 3; d++) {
		 transformedPoint[d] = inputPoint[d] + m_UseDisplacementAsTranslation * displacement[d] ;
	 }
   return transformedPoint ;
}

// Retrieve displacement at inputPoint
// Used to debug suspicious displacement field
template< class FieldData >
typename WarpTransform3D< FieldData >::OutputPointType 
WarpTransform3D< FieldData >
::GetDisplacement( const InputPointType & inputPoint ) const
{
   OutputPointType transformedPoint ;
   DeformationPixelType displacement ;
   itk::Index< 3 > index ;
   ...
   displacement = m_DeformationField->GetPixel( index ) ;
	 ...
	 for (int d = 0; d < 3; d++) {
		 transformedPoint[d] = -m_UseDisplacementAsTranslation * displacement[d] ;
	 }
   return transformedPoint ;
}
