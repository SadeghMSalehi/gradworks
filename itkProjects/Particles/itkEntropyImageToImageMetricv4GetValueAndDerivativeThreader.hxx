 ...
 *
 *=========================================================================*/
#ifndef __itkEntropyImageToImageMetricv4GetValueAndDerivativeThreader_hxx
#define __itkEntropyImageToImageMetricv4GetValueAndDerivativeThreader_hxx
...
void EntropyImageToImageMetricv4GetValueAndDerivativeThreader< TDomainPartitioner, TImageToImageMetric, TEntropyMetric>
::BeforeThreadedExecution()
{
  ...
  if( this->m_EntropyAssociate == NULL )
    {
...
template<class TDomainPartitioner, class TImageToImageMetric, class TEntropyMetric>
void
EntropyImageToImageMetricv4GetValueAndDerivativeThreader<TDomainPartitioner, TImageToImageMetric, TEntropyMetric>
::AfterThreadedExecution()
{
  ...
  if( ! this->m_EntropyAssociate->VerifyNumberOfValidPoints( this->m_EntropyAssociate->m_Value, *(this->m_EntropyAssociate->m_DerivativeResult) ) )
    {
    return;
    }

  itkDebugMacro("EntropyImageToImageMetricv4: NumberOfValidPoints: " << this->m_EntropyAssociate->m_NumberOfValidPoints);

  /* Accumulate the metric value from threads and store */
  this->m_EntropyAssociate->m_Value = NumericTraits<InternalComputationValueType>::Zero;
  ...
  if ( m2f2 <= NumericTraits<InternalComputationValueType>::epsilon() )
    {
    itkDebugMacro( "EntropyImageToImageMetricv4: m2 * f2 <= epsilon");
    return;
    }

  this->m_EntropyAssociate->m_Value = -1.0 * fm * fm / (m2f2);

  /* For global transforms, compute the derivatives by combining values from each region. */
  if( this->m_EntropyAssociate->GetComputeDerivative() )
    {
     ...
     */
    *(this->m_EntropyAssociate->m_DerivativeResult) += 2.0 *fm/(f2*m2)*(fdm - fm/m2*mdm);
    }
...
::ProcessVirtualPoint( const VirtualIndexType & virtualIndex, const VirtualPointType & virtualPoint, const ThreadIdType threadId )
{
  ...
  try
    ...
    if( pointIsValid &&
        this->m_EntropyAssociate->GetComputeDerivative() &&
        this->m_EntropyAssociate->GetGradientSourceIncludesFixed() )
      {
      this->m_EntropyAssociate->ComputeFixedImageGradientAtPoint( mappedFixedPoint, mappedFixedImageGradient );
      }
    }
  ...
  try
    ...
    if( pointIsValid &&
        this->m_EntropyAssociate->GetComputeDerivative() &&
        this->m_EntropyAssociate->GetGradientSourceIncludesMoving() )
      {
      this->m_EntropyAssociate->ComputeMovingImageGradientAtPoint( mappedMovingPoint, mappedMovingImageGradient );
      }
    }
  catch( ExceptionObject & exc )
    {
                ...
                const ThreadIdType                 threadID) const
{
  ...
  InternalComputationValueType f1 = fixedImageValue - this->m_EntropyAssociate->m_AverageFix;
  InternalComputationValueType m1 = movingImageValue - this->m_EntropyAssociate->m_AverageMov;
  ...
  if( this->m_EntropyAssociate->GetComputeDerivative() )
    ...
    this->m_EntropyAssociate->GetMovingTransform()->ComputeJacobianWithRespectToParameters(virtualPoint, jacobian);

    for (unsigned int par = 0; par < this->m_EntropyAssociate->GetNumberOfLocalParameters(); par++)
      {
