 ...
 *
 *=========================================================================*/
#ifndef _itkMyPowellOptimizer_cxx
#define _itkMyPowellOptimizer_cxx

#include "itkMyPowellOptimizer.h"

namespace itk {
MyPowellOptimizer::MyPowellOptimizer() {
...
void MyPowellOptimizer::SetLine(
		const MyPowellOptimizer::ParametersType & origin,
...
double MyPowellOptimizer::GetLineValue(double x) const {
	MyPowellOptimizer::ParametersType tempCoord(m_SpaceDimension);

	return this->GetLineValue(x, tempCoord);
}

double MyPowellOptimizer::GetLineValue(double x,
		ParametersType & tempCoord) const {
	for (unsigned int i = 0; i < m_SpaceDimension; i++) {
...
void MyPowellOptimizer::SetCurrentLinePoint(double x, double fx) {
	for (unsigned int i = 0; i < m_SpaceDimension; i++) {
		this->m_CurrentPosition[i] = this->m_LineOrigin[i]
				+ x * this->m_LineDirection[i];
	}
	if (m_Maximize) {
...
void MyPowellOptimizer::Swap(double *a, double *b) const {
	double tf;
...
void MyPowellOptimizer::Shift(double *a, double *b, double *c, double d) const {
	*a = *b;
	*b = *c;
	*c = d;
}
...
//
void MyPowellOptimizer::LineBracket(double *x1, double *x2, double *x3,
...
void MyPowellOptimizer::LineBracket(double *x1, double *x2, double *x3,
		double *f1, double *f2, double *f3, ParametersType & tempCoord) {
	//
...
void MyPowellOptimizer::BracketedLineOptimize(double ax, double bx, double cx,
		double fa, double functionValueOfb, double fc, double *extX,
		double *extVal) {
	MyPowellOptimizer::ParametersType tempCoord(m_SpaceDimension);

	this->BracketedLineOptimize(ax, bx, cx, fa, functionValueOfb, fc, extX,
			extVal, tempCoord);
}

void MyPowellOptimizer::BracketedLineOptimize(double ax, double bx, double cx,
		double itkNotUsed(fa), double functionValueOfb, double itkNotUsed(fc),
		double *extX, double *extVal, ParametersType & tempCoord) {
	double x;
	double v = 0.0;
	...
	for (m_CurrentLineIteration = 0;
		...
		} else /* x remains the better approx  */
		{
			...
			} else if (functionValueOft <= functionValueOfV || v == x
					|| v == w) {
				v = t;
...
void MyPowellOptimizer::StartOptimization() {
	...
	MyPowellOptimizer::ParametersType p(m_SpaceDimension);
	MyPowellOptimizer::ParametersType pt(m_SpaceDimension);
	MyPowellOptimizer::ParametersType ptt(m_SpaceDimension);
	...
	std::cout << "Line Origin: " << m_LineOrigin << std::endl;
	std::cout << "Line Direction: " << m_LineDirection << std::endl;

	fx = this->GetLineValue(0, tempCoord);

	for (m_CurrentIteration = 0; m_CurrentIteration <= m_MaximumIteration;
			m_CurrentIteration++) {
		fp = fx;
		ibig = 0;
		del = 0.0;
 ...
 *
 */
const std::string MyPowellOptimizer::GetStopConditionDescription() const {
 ...
 *
 */
void MyPowellOptimizer::PrintSelf(std::ostream & os, Indent indent) const {
