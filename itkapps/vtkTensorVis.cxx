...
static void constructGlyphs(vtkRenderer* ren) {
  ...
  pointData->SetNumberOfComponents(9);
  ...
  for (iter = indexVector.begin(); iter != indexVector.end(); iter++) {
    TensorImageType::IndexType roiIdx = *iter;
    roiIdx[1] = viewSliceNo;

    TensorType tensor = tensorImage->GetPixel(roiIdx);
    ...
    double tensorArray[9];
    int tensorIndex = 0;
    for (int r = 0; r < 3; r++) {
      for (int c = 0; c < 3; c++) {
        tensorArray[tensorIndex++] = tensor(r,c);
        cout << tensor(r,c) << ",";
      }
      cout << endl;
    }
    ...
    pointData->InsertNextTuple9(tensorArray[0], tensorArray[1], tensorArray[2], 
          tensorArray[3], tensorArray[4], tensorArray[5], 
          tensorArray[6], tensorArray[7], tensorArray[8]);
  ...
  grid->GetPointData()->SetTensors(pointData);
  ...
  glyph->SetScaleModeToScaleByTensor();
...
int main (int argc, char* argv[]) {
  ...
  tensorReader->Update();
  tensorImage = tensorReader->GetOutput();
