...
::ThreadedGenerateData( const OutputImageRegionType &outputRegionForThread,
                        int threadId)
{
	InputImagePointer inputPtr[5];
  ImageRegionConstIterator<TInputImage>* inputIt[5];
  ...
  outputIt.GoToBegin();
	...
	try {
		mu::Parser* thisParser = NULL;
		if (m_parsers[threadId] == NULL) {
			m_parsers[threadId] = new mu::Parser();
			thisParser = m_parsers[threadId];
			thisParser->DefineVar("A", &m_Vars[0+N_PARSERS*threadId]);
			thisParser->DefineVar("B", &m_Vars[1+N_PARSERS*threadId]);
			thisParser->DefineVar("C", &m_Vars[2+N_PARSERS*threadId]);
			thisParser->DefineVar("D", &m_Vars[3+N_PARSERS*threadId]);
			thisParser->DefineVar("E", &m_Vars[4+N_PARSERS*threadId]);
			thisParser->SetExpr(m_Equation);
		}
		while( !inputIt[0]->IsAtEnd() ) {
			for (int i = 0; i < nInputs; i++) {
				m_Vars[i+N_PARSERS*threadId] = static_cast<double>(inputIt[i]->Get());
				++(*inputIt[i]);
			}
			typename TOutputImage::PixelType o = 
				static_cast<typename TOutputImage::PixelType>(thisParser->Eval());
			outputIt.Set(o);
			++outputIt;
